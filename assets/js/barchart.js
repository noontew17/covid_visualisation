function barchart_cases(placement) {
    d3.csv("assets/data/countries_cases_geo.csv").then((data) => {
        d3.select(placement).html("");
        barchart_cases_render(data, placement)
    })
}

function barchart_cases_render(data, placement) {
    ...
    svg.selectAll(".bar")
        ...
        .on('mouseover', function (d, i) {
            tooltip
                .html(
                    `<div id ="cumbox"> ${d.location}</div><div>Cases: ${d.new_cases} <br> 
    ...
    svg.selectAll(".label")
        ...
        .enter().append("text")
        .attr("class", "")
        .attr("dx", function(d) {return x(d.new_cases) + 10})
        .attr("dy", function (d) {
            return y(d.location) + y.bandwidth()/2
        })
        .attr("alignment-baseline", "middle")
        .text(function(d){ return d3.format(",")(d.new_cases)})
